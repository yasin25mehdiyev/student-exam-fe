    ...
    <div class="flex flex-wrap items-center gap-3 pb-4">
      <div class="relative max-w-72 flex-1">
        ...
        <input
          ...
          [placeholder]="'students.table.searchPlaceholder' | translate"
      ...
      <table hlmTable>
        <thead hlmTHead>
          <tr hlmTr class="bg-brand-500 hover:bg-brand-500">
            <th hlmTh class="text-white">{{ 'students.fields.number' | translate }}</th>
            <th
              hlmTh
            ...
            >
              {{ 'students.fields.lastName' | translate }}
              @if (sortBy() === 'lastName') {
                <ng-icon
                  [name]="sortDirection() === 'asc' ? 'lucideChevronUp' : 'lucideChevronDown'"
                  size="14"
          ...
          @if (isLoadingSmooth()) {
            ...
            <tr hlmTr class="bg-white">
              <td hlmTd colspan="5" class="py-8 text-center text-ink-tertiary">
                {{ 'students.table.empty' | translate }}
              </td>
            </tr>
          } @else {
            @for (student of items(); track student.number) {
              <tr hlmTr class="bg-white">
